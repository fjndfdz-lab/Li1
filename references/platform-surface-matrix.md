...
| EvoLink | third-party aggregator surface | Unified Seedance 2.0 video generation API | Verified 2026-06-16 from EvoLink's public Seedance 2.0 page: EvoLink exposes Seedance 2.0 through an async video-generation API (`POST /v1/videos/generations`, poll `GET /v1/tasks/{task_id}`), Bearer token auth, per-second billing, 4-15s duration, 480p/720p/1080p quality options, and model values such as `seedance-2.0-text-to-video`. Treat endpoints, prices, model IDs, reference schema, callback behavior, and guardrails as EvoLink-specific and recheck before use. The repo endorses no reseller; listed for completeness. |
